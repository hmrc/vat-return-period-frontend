...
class ErrorSummarySpec extends ViewBaseSpec {
  ...
  val injectedView: ErrorSummary = injector.instanceOf[ErrorSummary]
  val form = ChooseDatesForm.datesForm
  val formWithError = ChooseDatesForm.datesForm.bind(Map("period-option" -> ""))
      ...
      "render the correct Html" in {
        ...
        val expectedMarkup = Html(
          s"""
             ...
             |    <li class="error-summary-list"> <a href="#period-option" id="period-option-error-summary" data-focuses="period-option">  Choose the new VAT Return dates </a> </li>
             |  </ul>
             |</div>
           """.stripMargin
        )
