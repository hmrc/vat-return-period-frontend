        ...
        case Some(affinityGroup) ~ allEnrolments => {
          (isAgent(affinityGroup), allEnrolments) match {
            ...
            case (_, _) =>
              ...
              Future.successful(Redirect(appConfig.manageClientUrl))   //TODO controllers.routes.CustomerCircumstanceDetailsController.show("non-agent")))
          }
        }
        case _ =>
...
//    } else {
//      Future.successful(Unauthorized("")) //TODO(views.html.errors.agent.agent_journey_disabled()))
//    }
  }
  ...
  private def checkAgentEnrolment[A](enrolments: Enrolments, f: AgentUser[A] => Future[Result])(implicit request: Request[A]) =
    if (enrolments.enrolments.exists(_.key == EnrolmentKeys.agentEnrolmentId)) {
      ...
      Future.successful(Forbidden("")) //TODO views.html.errors.agent.unauthorised()))
    }
}
